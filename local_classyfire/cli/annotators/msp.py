    ...
    *,
    ...
    with input_file.open("r", encoding="utf-8") as reader:
        with output_file.open("w", encoding="utf-8", newline="") as writer:
            annotate_records_streaming(
                records=iter_msp_records(reader),
...
def iter_msp_records(reader) -> Iterator[list[str]]:
    """Yield MSP records.

    MSP records are usually separated by blank lines.
    Blank separator lines are preserved at the end of each record.
    """
